package com.u3coding.audiovideo.aac
...
class AACEncoder(private val mFrameByte: ByteArray) : BaseEncoder() {
    ...
    override fun encode(path: String?, name: String?): Boolean {
        if (mWorker == null) {
            mWorker = Worker()
        }
        if (name != null) {
                ...
                fileInputStream = FileInputStream(
                    File(
                        Environment.getExternalStorageDirectory()
                            .absolutePath + "/temp.pcm"
                    )
                )
                fileOutputStream = FileOutputStream(File(path + name))
